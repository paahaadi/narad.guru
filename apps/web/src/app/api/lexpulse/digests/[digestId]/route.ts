...
import type { LexPulseDigestSummary } from "@/lib/workspaces/lexpulse-types";
...
export async function GET(
  ...
  const row = await queryRow<DigestRow>(
    session.tenantId,
    `
      SELECT
  ...
  const documentIds = asArray<Record<string, unknown>>(digest.documents)
    .map((document) => asOptionalString(document.document_id))
    .filter((documentId): documentId is string => Boolean(documentId));

  const documents = documentIds.length
    ? await queryRows<DocumentRow>(
        session.tenantId,
        `
          SELECT
          ...
          WHERE d.tenant_id = $1
            AND d.id = ANY($2::uuid[])
        `,
        [session.tenantId, documentIds],
      )
    : [];

  const documentsById = new Map(documents.map((document) => [document.document_id, document]));
  const evidence = documentIds
    .map((documentId) => documentsById.get(documentId))
    .filter((document): document is DocumentRow => Boolean(document))
    .map((document) => toEvidence(document, regulator, affectedSectors))
    .slice(0, 4);

  const detail: LexPulseDigestSummary = {
    ...
    effectiveDate:
      asOptionalString(digest.effective_date) ??
